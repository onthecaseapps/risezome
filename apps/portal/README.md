...
The hosted Upwell portal — Next.js 16 App Router app on Vercel. Hosts both the
...
pnpm --filter @upwell/portal dev   # http://localhost:3000
```
...
6. Verify the domain you'll deploy at (`upwell.com`) in Google Cloud Console
...
```bash
pnpm --filter @upwell/portal typecheck
pnpm --filter @upwell/portal test
pnpm --filter @upwell/portal lint
pnpm --filter @upwell/portal build
```

The RLS tests (`test/rls/*.test.ts`) require a running `supabase start`
local stack and skip otherwise. Set `UPWELL_RUN_RLS_TESTS=1` to require
