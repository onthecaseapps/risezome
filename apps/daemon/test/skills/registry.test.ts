...
describe('SkillRegistry', () => {
  it('registers two skills and lists them in insertion order', () => {
    ...
    expect(r.list().map((s) => s.name)).toEqual(['github.count', 'github.list']);
  });

  it('looks up a registered skill by name', () => {
    const r = new SkillRegistry();
    r.register(makeSkill('github.count'));
    expect(r.lookup('github.count')?.name).toBe('github.count');
  });
...
describe('formatAsSource', () => {
  it('produces a SynthesisSource with rank:0 for a count result', () => {
    ...
    const src = formatAsSource(result, 'github.count', { state: 'open', type: 'issue' });
    expect(src.rank).toBe(0);
    expect(src.title).toBe('Tool: github.count({"state":"open","type":"issue"})');
  ...
  it('renders numbered items beneath the summary for a list result', () => {
    ...
    const src = formatAsSource(result, 'github.list', { state: 'open', limit: 3 });
    expect(src.rank).toBe(0);
