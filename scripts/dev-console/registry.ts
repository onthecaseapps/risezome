...
export function appRegistry(repoRoot: string, tag: string): ProcDef[] {
  ...
  return [
    { name: 'portal', command: 'pnpm', args: ['--filter', '@risezome/portal', 'dev'], cwd: repoRoot, order: 2 },
    { name: 'inngest', command: 'npx', args: ['inngest-cli@latest', 'dev'], cwd: repoRoot, order: 3 },
    { name: 'bot-worker', command: 'pnpm', args: ['--filter', '@risezome/bot-worker', 'dev'], cwd: repoRoot, order: 4 },
    { name: 'tunnel', command: 'cloudflared', args: ['tunnel', '--config', tunnelConfig, 'run'], cwd: repoRoot, order: 5 },
  ];
}
