...
alter table public.user_google_tokens add column if not exists key_org_id uuid;
...
alter table public.trello_connections add column if not exists token_version int not null default 1;
...
-- NOT NULL DEFAULT 1 (legacy): existing transcript rows written before this column
-- existed must be visible to the U11 backfill, whose filter is `version < 2`. A
-- NULLable column would make `transcript_key_version < 2` evaluate to NULL for
-- those rows (three-valued logic), silently excluding every legacy transcript from
-- the backfill so it could never converge. Non-transcript events default to 1 too
-- but carry no transcript_text_enc, so the backfill (which only touches rows with
-- ciphertext) skips them; new transcript rows are stamped 2 on write.
alter table public.meeting_events add column if not exists transcript_key_version int not null default 1;
